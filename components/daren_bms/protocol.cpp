...
std::string build_command(uint8_t device_address, uint8_t cid2, uint8_t module_id) {
  switch (cid2) {
    case CMD_PARAMS:
      return build_command(device_address, cid2,
                           StaticVector<uint8_t, BUF_MAX_SIZE>{device_address, 0x01, module_id, 0xff, 0x00});
    case CMD_MFG_INFO:
...
CapParams unpack_cap_params(StaticVector<uint8_t, BUF_MAX_SIZE> payload) {
  ...
  result.total_charge_cap_ah =
      static_cast<float>(payload[12] << 8 | payload[13] << 8 | payload[14] << 8 | payload[15]) / 100.0f;
  result.total_discharge_cap_ah =
      static_cast<float>(payload[16] << 8 | payload[17] << 8 | payload[18] << 8 | payload[19]) / 100.0f;
